...
public interface UserDao {
    ...
    //通过用户名和产品实例id查询用户信息,不返回用户对应的角色、模块和Actions信息
    @Select("SELECT * from t_user where username = #{username} and product_inst_id = #{productInstId} limit 1")
    @Results({
            @Result(id=true, column="user_id", property="userId"),
            @Result(column="product_inst_id", property="productInstId"),
            @Result(column="username", property="username"),
    ...
    public User findByUsernameAndProductInstId(@Param("username") String username,@Param("productInstId") String productInstId);
    ...
    //用于系统管理，更改账号特定信息
    @Update("Update t_user set mobile_no = #{mobileNo}, email = #{email}, password = #{password}, account_locked = #{accountLocked} where user_id = #{userId} limit 1")
